...
def old_preprocess(file,destination,depth): 
    try:
    ...
    return {'result':result, 'cardinality':cardinality}
...
def uploader():
        ...
        for file in files:
            name = file.filename
            names.append(name)
            #file.save(rf"D:\XVIII\full_reference\backend\virtual\app\docx_files\{name}") 
...
def deleter():
    ...
    if directive == 'remove':
        #existent_files = os.listdir(r'D:\XVIII\full_reference\backend\virtual\app\docx_files')
        existent_files = os.listdir(rf'{directory}\docx_files\{CURRENT_USER}')
        for file in existent_files:
            #os.remove(rf'D:\XVIII\full_reference\backend\virtual\app\docx_files\{file}')
...
@applet.route('/old_referencer', methods=['GET','POST'])
def old_referencer():
    print('Ты прошёл!')
    database = sqlite3.connect('actual_references.db')
    cur = database.cursor()
...
def uploading():
        ...
        for file in files:
            name = file.filename
            names.append(name)
            #file.save(rf"D:\XVIII\full_reference\backend\virtual\app\referencer_docx\{name}") 
                ...
                if raw_text.startswith('Tilde'):
                        ...
                        for pair in pairs:
                            try:
                                ...
                                #raw_text = raw_text.replace(rf'pair["was"]',rf'{pair["will"]}')
                            except:
                                print(pair)
...
def vectorize():
    try:
        ...
        print(err)
        return {'type':'fail','message':'Проверьте, что указали все характеристики модели и попробуйте снова!'}
